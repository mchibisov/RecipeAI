...
def run():
    init_db()
    # Хостинг сам передает нужный порт через переменную окружения PORT.
    # Если её нет (запуск локально), берем стандартный PORT из core.py.
    port = int(os.environ.get("PORT", PORT))

    # host="0.0.0.0" сообщает Flask, что нужно слушать ВСЕ доступные сетевые интерфейсы,
    # а не только локальный петлевой адрес 127.0.0.1.
